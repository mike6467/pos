...
async def verify(phone: str = Form(...), code: str = Form(...), password: str = Form("")):
    ...
    try:
        ...
        try:
            ...
            pass
        return HTMLResponse(f"<h3>❌ Authentication Failed</h3><p>Error: {str(e)}</p><p><strong>If the password is correct but still failing:</strong> Your Telegram account may need to disable/re-enable 2FA in Telegram settings.</p><p><a href='/'>Back to start</a></p>")
