...
async def send(caption: str = Form(...), photos: list[UploadFile] = File(...)):
    ...
    return HTMLResponse(f"<h3>✓ Started posting {len(photo_paths)} images to all your groups!</h3><p>Posts will be sent with 5-second intervals, then 30-minute wait between cycles.</p><p><a href='/'>Back home</a></p>")
